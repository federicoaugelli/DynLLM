...
def test_transformers_accepts_speech_model_type() -> None:
    model = ModelConfig(
        name="speecht5-hf",
        path=Path("microsoft/speecht5_tts"),
        backend=BackendType.transformers,
        model_type=ModelType.speech,
        device="xpu",
        vram_mb=2000,
    )
    assert model.model_type == ModelType.speech
...
def test_transformers_rejects_unsupported_model_type() -> None:
    with pytest.raises(
        ValueError,
        match="transformers supports model_type=llm, transcription, and speech",
    ):
        ModelConfig(
            name="bad-embedding",
            path=Path("/tmp/bad-embedding"),
            backend=BackendType.transformers,
